  ...
  <div class="w-full">
    ...
    <div class="relative">
      <div v-if="$slots.prefix || prefixIcon" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2">
        <component v-if="prefixIcon" :is="prefixIcon" class="h-5 w-5 text-gray-400 dark:text-neutral-500" />
      ...
      <div v-if="$slots.suffix || suffixIcon" class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2">
        <component v-if="suffixIcon" :is="suffixIcon" class="h-5 w-5 text-gray-400 dark:text-neutral-500" />
...
const baseClasses = 'w-full h-10 px-3 rounded-lg border focus-ring disabled:opacity-50 disabled:cursor-not-allowed dark:[color-scheme:dark]'
const stateClasses = computed(() => {
  if (props.error) return 'border-red-300 bg-red-50 dark:border-red-700 dark:bg-red-950'
  return 'border-gray-300 bg-white dark:border-neutral-600 dark:bg-neutral-800'
})
...
const inputClasses = computed(() =>
  cn(
    ...
    'text-gray-900 placeholder:text-gray-400 dark:text-neutral-100 dark:placeholder:text-neutral-500'
  )
)
</script>
