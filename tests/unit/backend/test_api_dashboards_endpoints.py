...
class TestCreateDashboardEndpoint:
    ...
    def test_create_returns_201_fields(self, db, user_a):
        ...
        result = create_dashboard(payload=payload, db=db, current_user=user_a)
    ...
    def test_create_assigns_user_id(self, db, user_a, user_b):
    ...
    def test_create_default_empty_widgets(self, db, user_a):
...
class TestListDashboardsEndpoint:
    ...
    def test_list_returns_only_own_dashboards(self, db, user_a, user_b):
        ...
        results_a = list_dashboards(db=db, current_user=user_a)
        results_b = list_dashboards(db=db, current_user=user_b)
    ...
    def test_list_empty(self, db, user_a):
        """No dashboards -> empty list."""
        results = list_dashboards(db=db, current_user=user_a)
        assert results == []
...
class TestGetDashboardEndpoint:
    ...
    def test_get_own_dashboard(self, db, user_a):
        """User can retrieve their own dashboard."""
        d = _make_dashboard(db, user_a.id, title="My Board")
        result = get_dashboard(dashboard_id=d.id, db=db, current_user=user_a)
        assert result.title == "My Board"

    def test_get_other_users_dashboard_returns_404(self, db, user_a, user_b):
        """User cannot retrieve another user's dashboard."""
        d = _make_dashboard(db, user_b.id, title="Not Yours")
        with pytest.raises(HTTPException) as exc_info:
            get_dashboard(dashboard_id=d.id, db=db, current_user=user_a)
        assert exc_info.value.status_code == 404

    def test_get_nonexistent_returns_404(self, db, user_a):
        """Non-existent ID -> 404."""
        with pytest.raises(HTTPException) as exc_info:
            get_dashboard(dashboard_id=99999, db=db, current_user=user_a)
        assert exc_info.value.status_code == 404
...
class TestUpdateDashboardEndpoint:
    ...
    def test_partial_update_title(self, db, user_a):
        ...
        result = update_dashboard(dashboard_id=d.id, payload=payload, db=db, current_user=user_a)

        assert result.title == "New"
        assert result.description == "Keep"
        assert result.widgets == [{"v": 1}]

    def test_update_other_users_dashboard_returns_404(self, db, user_a, user_b):
        """Cannot update another user's dashboard."""
        d = _make_dashboard(db, user_b.id, title="Bob's")
        payload = DashboardUpdate(title="Hijack")
        with pytest.raises(HTTPException) as exc_info:
            update_dashboard(dashboard_id=d.id, payload=payload, db=db, current_user=user_a)
        assert exc_info.value.status_code == 404

    def test_update_nonexistent_returns_404(self, db, user_a):
        """Non-existent ID -> 404."""
        payload = DashboardUpdate(title="Ghost")
        with pytest.raises(HTTPException) as exc_info:
            update_dashboard(dashboard_id=99999, payload=payload, db=db, current_user=user_a)
        assert exc_info.value.status_code == 404
...
class TestDeleteDashboardEndpoint:
    ...
    def test_delete_removes_dashboard(self, db, user_a):
        """Deleted dashboard is removed from DB."""
        d = _make_dashboard(db, user_a.id, title="Delete Me")
        did = d.id
        delete_dashboard(dashboard_id=did, db=db, current_user=user_a)

        assert db.query(Dashboard).filter(Dashboard.id == did).first() is None

    def test_delete_other_users_dashboard_returns_404(self, db, user_a, user_b):
        """Cannot delete another user's dashboard."""
        d = _make_dashboard(db, user_b.id, title="Protected")
        with pytest.raises(HTTPException) as exc_info:
            delete_dashboard(dashboard_id=d.id, db=db, current_user=user_a)
        assert exc_info.value.status_code == 404
        # Dashboard still exists
        assert db.query(Dashboard).filter(Dashboard.id == d.id).first() is not None

    def test_delete_nonexistent_returns_404(self, db, user_a):
        """Non-existent ID -> 404."""
        with pytest.raises(HTTPException) as exc_info:
            delete_dashboard(dashboard_id=99999, db=db, current_user=user_a)
        assert exc_info.value.status_code == 404

    def test_delete_returns_none(self, db, user_a):
        """delete_dashboard returns None (maps to 204 No Content)."""
        d = _make_dashboard(db, user_a.id, title="Return None")
        result = delete_dashboard(dashboard_id=d.id, db=db, current_user=user_a)
